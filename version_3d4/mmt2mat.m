function [MMT,MMT_Site_Info,MMT_Transects,MMT_Field_Config,...
          ...
          MMT_MB_Field_Config,MMT_MB_Active_Config] = mmt2mat(infile)
            ...
            % for whatever reason there is no Locked field for Karen Hardys
            % 05PH003_20080603.mmt file, commented
            % SAM 10juillet2014, the original code gave me an error when
            % there was none so I changed isstruct to isfield. if there's
            % no field it doesn't know if that field is a structure
 
             %if isstruct(s.WinRiver.Project.Locked)
             if isfield(s.WinRiver.Project, 'Locked')
        ...
        % of configuration variables in the structure to be constructed through multiple calls.
            ...
            % to
            if isfield(rec, 'Root_Directory.Text')
